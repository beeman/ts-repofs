 ...
 */
export function updateWorkingState(repoState, branch, newWorkingState) {
 ...
 */
export function updateCurrentWorkingState(repoState, workingState) {
  return updateWorkingState(
    repoState,
    repoState.getCurrentBranch(),
    workingState,
  )
}
 ...
 */
export function fetchTree(repoState, driver, branch) {
  // Fetch a working tree for this branch
  return fetch(driver, branch).then(newWorkingState => {
 ...
 * @return {RepositoryState}
 */
export function checkout(repoState, branch) {
  if (!(branch instanceof Branch)) {
    branch = repoState.getBranch(branch)
    if (branch === null) {
  ...
  if (!repoState.isFetched(branch)) {
    throw Error(
      'Tree for branch ' + branch.getFullName() + ' must be fetched first',
    )
  }
  return repoState.set('currentBranchName', branch.getFullName())
}
 ...
 */
export function fetchBranches(repoState, driver) {
 ...
 * @return {Promise<RepositoryState>}
 */
export function initialize(driver) {
  const repoState = RepositoryState.createEmpty()
  return fetchBranches(repoState, driver).then(repoState => {
    ...
    return fetchTree(repoState, driver, branch).then(repoState => {
 ...
 */
export function syncFilesystem(driver, branch) {
 ...
 */
export function isClean(repoState) {
  return repoState
    .getWorkingStates()
