...
import { fetchTree } from './repo'
import { getMergedTreeEntries } from './working'
...
function prepare(repoState, opts) {
  ...
  // Parent SHA
  opts.parents = List([workingState.getHead()])

  // Get merged tree (with applied changes)
  opts.treeEntries = getMergedTreeEntries(workingState)
 ...
 */
function flush(repoState, driver, commitBuilder, options: any = {}) {
  ...
  return (
    driver
      ...
      .then(commit => {
        return (
          driver
            .forwardBranch(options.branch, commit.getSha())
      ...
      .then(function updateWorkingState(forwardedRepoState) {
        const forwardedBranch = forwardedRepoState.getBranch(
          options.branch.getFullName(),
        )
        return fetchTree(forwardedRepoState, driver, forwardedBranch)
      })
  )
}
